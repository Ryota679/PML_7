...
class _TenantManagementPageState extends ConsumerState<TenantManagementPage> {
  ...
  Widget? _buildFloatingActionButton(BuildContext context) {
    ...
    // Phase 3: Free tier (trial expired) cannot create tenant
    if (user.isFreeTier) {
      // Show disabled FAB that triggers upgrade dialog
      return FloatingActionButton.extended(
        onPressed: () => _showPhase3UpgradeDialog(context),
        icon: const Icon(Icons.lock),
        label: const Text('Tambah Tenant (Premium)'),
        backgroundColor: Colors.grey,
      );
    }
    
    // Free tier (active trial) can create up to 2 tenants
    if (user.isFree) {
      // Count actual tenants from state (more reliable)
      final currentCount = tenantState.tenants.length;
      // Hide FAB if already have 2 or more tenants
      if (currentCount >= 2) {
        return null;
      }
    }
